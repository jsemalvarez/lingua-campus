...
import { resetStudentPassword, softDeleteStudent, hardDeleteStudentAction } from "./actions";
import { useRouter } from "next/navigation";

export function StudentDangerZone({ studentId, studentStatus }: { studentId: string; studentStatus?: string }) {
    ...
    const isPreInscribed = studentStatus === "PRE_INSCRIBED";
    ...
    const handleDelete = async () => {
        const confirmMsg = isPreInscribed
            ? "⚠️ Al ser una pre-inscripción, esta acción ELIMINARÁ PERMANENTEMENTE todos los datos de este alumno.\n\nEsta acción NO se puede deshacer. ¿Continuar?"
            : "⚠️ ¿Estás seguro de que querés eliminar este estudiante?\n\nEl estudiante será dado de baja (borrado lógico). Sus pagos e historial académico se conservarán para mantener la integridad de las métricas del instituto.\n\nEsta acción se puede revertir.";
        ...
        const res = isPreInscribed 
            ? await hardDeleteStudentAction(studentId)
        ...
        <Card className="mt-8 border-red-500/30 bg-red-500/5">
            <div className="p-6">
                ...
                <div className="space-y-6">
                    {/* Restablecer Contraseña (Oculto para Pre-inscriptos) */}
                    {studentStatus !== "PRE_INSCRIBED" && (
                        <>
                            <div>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div>
                            <h4 className="font-semibold text-foreground">
                                {isPreInscribed ? "Borrar Inscripción" : "Eliminar Estudiante"}
                            </h4>
                            <p className="text-sm text-muted-foreground mt-1">
                                {isPreInscribed 
                                    ? "Eliminar permanentemente todos los datos de esta pre-inscripción."
                                    : "Dar de baja al estudiante (borrado lógico). Sus pagos y notas se conservan."}
                            </p>
                        </div>
                        <Button
                            variant="destructive"

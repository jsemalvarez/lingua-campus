...
export function EditGuardianForm({ guardian, onCancel }: { guardian: GuardianData, onCancel: () => void }) {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        startTransition(async () => {
            const result = await updateGuardianAction(formData);
            if (result.success) {
                setStatus("success");
                setTimeout(() => {
                    onCancel();
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6 bg-card border border-border/50 p-6 rounded-xl shadow-sm animate-in fade-in slide-in-from-top-2">
            <div className="flex items-center justify-between pb-4 border-b border-border/40">
                <h3 className="text-xl font-bold">Editar Tutor</h3>
            ...
            <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1.5 focus-within:text-blue-600 transition-colors">
                        ...
                        <input type="text" name="name" defaultValue={guardian.name} required className="w-full px-4 py-2 rounded-lg border border-input transition-all bg-background text-foreground text-sm font-medium focus:ring-2 focus:ring-primary/20 outline-none" />
                    </div>
                    <div className="space-y-1.5 focus-within:text-blue-600 transition-colors">
                        <label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Correo Electrónico</label>
                        <input type="email" name="email" defaultValue={guardian.email} disabled className="w-full px-4 py-2 rounded-lg border border-input transition-all bg-background text-foreground text-sm font-medium opacity-50 cursor-not-allowed" />
                        <p className="text-[10px] text-muted-foreground">El correo se usa como acceso principal y no puede editarse aquí.</p>
                    </div>
                    <div className="space-y-1.5 focus-within:text-blue-600 transition-colors">
                        <label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Celular Confirmado</label>
                        <input type="tel" name="phone" defaultValue={guardian.phone || ""} className="w-full px-4 py-2 rounded-lg border border-input transition-all bg-background text-foreground text-sm font-medium focus:ring-2 focus:ring-primary/20 outline-none" placeholder="+54 9 11..." />
                    </div>
                </div>
            </div>
